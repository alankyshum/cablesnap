...
### Canonical F-Droid Purity Strategy: Source-Level GMS/MLKit Neutralization (Not R8 -dontwarn)
**Source**: BLD-4420 — F-Droid PR consolidation (churn from BLD-4411)
**Date**: 2026-07-27
**Context**: 11+ open PRs converged on the F-Droid GMS/MLKit purity problem with two competing strategies: (A) add R8 `-dontwarn` rules to swallow missing proprietary classes at minify time (#883, #884, #885, #887, #890), or (B) neutralize GMS/MLKit at the Expo source-generation step before R8 ever sees them (#889 disable expo-camera MLKit, #892 robust MLKit/GMS neutralization, #893 post-prebuild sanitize). Approach B landed on main as 3783e80 (BLD-4399) and superseded approach A. The R8-rules PRs became obsolete because there are no more missing-class references to warn about once the source is sanitized.
**Learning**: For an F-Droid variant of an app that pulls in proprietary Google dependencies through libraries (expo-camera → MLKit), fix impurity at the SOURCE (Expo prebuild / gradle generation / source graph) rather than at the linker/minifier (R8). Source-neutralization removes the offending code paths entirely, is verifiable by strings/dex inspection, and works whether R8 is enabled or not. R8 `-dontwarn` only silences warnings — it does not remove the proprietary references, so purity gates that inspect the APK still fail. Only one strategy should be canonical; keeping both open invites churn and merge conflicts.
**Action**: F-Droid purity fixes MUST go through the source-neutralization pipeline (strip proprietary Expo deps → disable Expo Camera barcode graph → post-prebuild sanitize). The per-PR F-Droid purity gate (#888) is the guard that prevents regression. Do NOT reopen R8 `-dontwarn`-based approaches without first proving that source-neutralization cannot cover the case. When multiple PRs converge on the same problem, techlead consolidates to one owner + one approach before more code is written.
**Tags**: fdroid, gms, mlkit, r8, expo, android, build, purity, consolidation, architecture
