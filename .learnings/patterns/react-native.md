...
### Track fetchedId in useReducer to Prevent Stale Data on Rapid Prop Changes
**Source**: BLD-402 — Personal Exercise Stats in Workout Detail Drawer
**Date**: 2026-04-19
**Context**: The ExerciseDetailDrawer opens for different exercises in rapid succession. The async data hook (`useExerciseDrawerStats`) fetches 3 parallel queries per exercise ID. Without identity tracking, data from a previous exercise could display briefly for the current exercise if the user switches faster than the fetch completes.
...
### Fixed-Width Label Containers Truncate on Varying Screen Sizes — Use flexShrink: 0
**Source**: BLD-418, BLD-421, BLD-427 — Exercise creation UX / Input label truncation / Weekday text truncation
**Date**: 2026-04-20
**Context**: The Input component used `width: 120` on label containers and `numberOfLines={1}` with `ellipsizeMode='tail'` on label text. The WeeklySchedule used `width: 44` on day labels. Both worked on the developer's device but truncated labels on the Samsung Z Fold6, making text like "Exercise Name" unreadable.
**Learning**: Fixed-width constraints on text label containers (`width: N`) silently truncate text when font scale, screen density, or device dimensions differ from the development device. This is especially prevalent on foldable devices where the same app runs in both narrow and wide configurations. The truncation is invisible during development if the developer's device happens to fit the fixed width. Using `numberOfLines={1}` compounds the problem by adding programmatic truncation on top of layout truncation.
**Action**: Never use `width: N` on containers whose children are text labels. Use `flexShrink: 0` to prevent the container from shrinking below its content, and `minWidth: N` if a minimum size is needed for alignment. Remove `numberOfLines={1}` and `ellipsizeMode` from labels that should always display in full. During code review, flag any `width:` style on a View that contains only Text children — it's a truncation risk.
**Tags**: react-native, flexbox, fixed-width, flexShrink, truncation, label, foldable, samsung-z-fold, responsive, numberOfLines, code-review

### Partial<Record> Override Pattern for Per-Entity Customizable Defaults
**Source**: BLD-431 — Per-Muscle Volume Landmarks with Customization
**Date**: 2026-04-20
**Context**: Replacing a hardcoded global constant (MRV=20 for all muscle groups) with evidence-based per-entity defaults that users can customize and persist.
**Learning**: Store only user overrides as `Partial<Record<EntityKey, Config>>` in a key-value store. At read time, merge overrides onto a complete defaults object using a `key in defaults` guard, which silently ignores unknown keys (forward compatibility if the enum changes). This avoids storing redundant default values and makes future default updates propagate automatically to non-customized entities.
**Action**: When adding user-customizable per-entity settings, create a pure `mergeWithDefaults(custom)` function that spreads defaults then overlays matching custom keys. Store only the delta. Use `parseCustom()` with try/catch + type guards (not-null, not-array, typeof === object) around JSON.parse for defensive deserialization. Audit every calculation that used the old hardcoded constant — dynamic per-entity values change aggregation logic (e.g., maxSets must now use `Math.max(...allEntityValues)` instead of a flat constant).
**Tags**: partial-record, override-pattern, user-customization, settings, forward-compatibility, merge-defaults, app-settings

### Use Bottom Sheets for User Input in FlatList Headers to Avoid Scroll Jumps
**Source**: BLD-432 — PLAN: Per-Exercise Strength Goals (Phase 66)
**Date**: 2026-04-20
**Context**: The Phase 66 plan initially placed a "Set Goal" inline form in the FlatList header of the exercise detail screen. UX review flagged that expanding an inline form inside a FlatList/ScrollView header causes the content size to change, triggering visible scroll position jumps as FlatList recalculates layout.
**Learning**: FlatList recalculates its content offset whenever the header component's height changes. An inline form that expands (e.g., showing input fields on button tap) inside `ListHeaderComponent` causes the entire list to jump as FlatList adjusts scroll position to accommodate the new header height. This is jarring and disorienting for users, especially on screens with existing scroll content below.
**Action**: When adding user input forms to screens that use FlatList or ScrollView, present the form in a `@gorhom/bottom-sheet` overlay instead of expanding it inline in the header. This keeps the list scroll position stable. Use the existing bottom sheet infrastructure (snap points, backdrop, keyboard avoidance) rather than building inline expand/collapse animations.
**Tags**: flatlist, bottom-sheet, scroll-jump, header-height, ux, form-input, gorhom, layout-shift

### Wire State Transition Triggers — CRUD + UI Is Not a Complete Feature
**Source**: BLD-433 — Per-Exercise Strength Goals (Phase 66)
**Date**: 2026-04-20
**Context**: The strength goals implementation included `achieveGoal()` in the DB layer, the hook exposed it, and GoalProgressCard rendered the achieved state (`✅ Achieved on [date]`). But nothing in any user-facing flow — session completion, PR detection, or workout logging — actually called `achieveGoal()`. Goals could never transition from active to achieved.
**Learning**: Building CRUD operations and rendering all entity states in the UI does not make a feature complete. Multi-state entities (active→achieved, draft→published, pending→confirmed) require explicit TRIGGER WIRING: code at the action point (e.g., PR detection in `useSessionActions`) that checks the transition condition and calls the transition function. This is easy to miss because the DB layer, hook layer, and UI layer each appear complete in isolation — the gap is the cross-layer integration that connects a user action to a state change.
**Action**: When implementing a multi-state entity, enumerate every state transition in the design (active→achieved, pending→cancelled, etc.) and explicitly identify WHERE in the user flow each transition is triggered. During self-review, verify each transition function is actually called from a user-reachable code path. Add a test that exercises the full flow (user action → state check → transition call → UI update), not just the transition function in isolation.
**Tags**: state-machine, state-transition, wiring, integration, crud, multi-state, cross-layer, completeness

### Wrap Non-Critical Promise.all Members with Individual .catch() Fallbacks
**Source**: BLD-436 — PLAN: Phase 67 — Estimated Workout Duration on Template Cards
**Date**: 2026-04-20
**Context**: Quality Director review of a plan to add duration estimates to `loadHomeData()` identified that a new query inside `Promise.all` would cause the entire home screen to fail if the query throws. The duration badge is enhancement data — the home screen must render without it.
...
### Remove getItemLayout When Adding ListHeaderComponent to FlatList
**Source**: BLD-453 — Quick-Add Exercises from Recent History
**Date**: 2026-04-20
**Context**: Adding a `ListHeaderComponent` (Recent/Frequent exercise sections) to an existing FlatList that used `getItemLayout` for scroll optimization. The `getItemLayout` callback calculated each item's offset as `ITEM_HEIGHT * index`, assuming items start at offset 0 with uniform height.
**Learning**: `getItemLayout` assumes items begin at offset 0. When `ListHeaderComponent` is present, the header adds variable-height content above the first item, shifting all offsets. The stale offset calculations cause scroll-to-index failures, visual jumps, and incorrect virtualization boundaries. For lists under ~200 items, removing `getItemLayout` entirely is acceptable — React Native will measure items dynamically. For large lists, compute the header height and add it to each item's offset: `offset: HEADER_HEIGHT + ITEM_HEIGHT * index`.
**Action**: When adding `ListHeaderComponent` to a FlatList that uses `getItemLayout`, remove or update `getItemLayout`. If the list is small (<200 items), remove it. If scroll-to-index or virtualization performance matters, measure the header height and incorporate it into the offset calculation. Never leave the original `getItemLayout` unchanged after adding a header.
**Tags**: react-native, flatlist, getItemLayout, ListHeaderComponent, virtualization, scroll-offset, layout

### Widen Batch DB Query and Pre-Filter at Consumer for Feature Addition
**Source**: BLD-456 — Smart Weight Progression Suggestions (Phase 74)
**Date**: 2026-04-20
**Context**: The progression feature needed to know whether ALL previous sets (including incomplete ones) were completed, but the existing `getPreviousSetsBatch` query filtered to `completed=1` in SQL. The prefill feature only needed completed sets.
...
### Every In-Session setInterval Must Pause on AppState=background
**Source**: BLD-577 — Battery drain + crash during workout session (GitHub #336)
**Date**: 2026-04-24
**Context**: User reported ~17% / hr battery drain on Z Fold6 with app v0.26.7. Audit of the session screen found ≥5 persistent `setInterval`/keep-awake sites; only `useSessionActions.ts` and `useRestTimer.ts` paused on `AppState` → `background` (per the BLD-553 precedent). `useSetTimer.ts` kept a 1Hz interval running while backgrounded and `app/session/[id].tsx` called `activateKeepAwakeAsync()` on mount without a matching `deactivateKeepAwake()` on unmount — leaking the wake-lock every time the user left the session screen.
**Learning**: On some RN / Hermes runtimes, `setInterval` continues to schedule wake-ups even when the JS thread is mostly idle in the background; at best it forces React to re-render on resume when `Date.now()` jumps. A `setInterval`-based stopwatch must always: (1) store the start timestamp in a ref, (2) clear the interval on `AppState` `background`/`inactive`, (3) on `active` recompute elapsed from the absolute start ref (no drift from accumulated ticks), (4) re-arm the interval. Similarly, `activateKeepAwakeAsync()` without a matching `deactivateKeepAwake()` in the effect cleanup leaks the screen-on lock across navigations and compounds drain.
**Action**: For any new `setInterval` inside a session/workout hook, replicate the BLD-553 pattern from `hooks/useSessionActions.ts` (start-guarded on `AppState.currentState === 'active'`, AppState listener pauses/restarts, unmount cleanup clears the interval). For any `activateKeepAwakeAsync()`, pair it with `deactivateKeepAwake()` in the same `useEffect` cleanup and guard the release with a flag that flips on activation failure. Emit `sessionBreadcrumb()` at every lifecycle event (open, close, keepawake acquire/release, AppState transitions, timer start/stop) — scalar-only data — so the next captured Sentry event is diagnosable. Regression-lock source-scan tests (`__tests__/app/session-keep-awake-lifecycle.test.ts` and similar) catch future regressions.
**Tags**: setInterval, appstate, keep-awake, battery-drain, z-fold, sentry-breadcrumbs, session-screen, bld-553-pattern

### CVD-Immune Intensity Encoding for Low-Cardinality Heatmaps
**Source**: BLD-732 — Workout history heatmap intensity legend a11y; PR #410
**Date**: 2026-04-28
**Context**: The 16-week workout heatmap had 4 intensity steps (0/1/2/3+) using a single-hue alpha ramp. Steps 1 and 2 were perceptually identical for users with deuteranopia/protanopia and even normal-vision users at small cell sizes. Color-only differentiation between adjacent low-frequency steps was the failure mode — the chart's intent (training-volume trend) collapsed into a binary "any workout vs 3+ workouts" signal.
**Learning**: For heatmaps with ≤5 discrete intensity steps, **single-hue lightness ramp + numeric labels in each cell** is a CVD-immune encoding. The fix is two channels: (1) photometric lightness steps that survive grayscale conversion (use explicit L\* targets from the design system, not "darker by 1 alpha unit" — linear alpha ≠ linear perceived lightness), (2) a redundant non-color cue (numeric overlay, dot size, or fill pattern). GitHub's contribution graph is the reference pattern: each level is perceptually distinct in lightness AND labeled on hover. By-construction photometric reasoning satisfies CVD ACs **only** when the property holds for *any hue across the entire input space* — a single-hue α-ramp qualifies because the proof references no specific RGB; "we picked these three colors because they look distinct in our palette" does not.
**Action**: When designing or auditing any low-cardinality heatmap/intensity chart: (1) define lightness targets per step in design tokens, (2) add an in-cell numeric label or equivalent redundant cue, (3) verify with a grayscale conversion pass (no renderer needed when the encoding is single-hue), (4) if the encoding is multi-hue or palette-specific, require empirical CVD-emulation captures (deutan/protan/tritan) — by-construction reasoning does not apply.
**Tags**: a11y, cvd, color-blindness, heatmap, intensity-encoding, design-tokens, lightness-ramp, redundant-cues, cross-project
