...
### Mock-Based DB Tests Cannot Gate Query Refactoring
**Source**: BLD-369 — Migrate database layer to Drizzle ORM for type safety
**Date**: 2026-04-19
**Context**: A 5-phase Drizzle ORM migration plan proposed refactoring ~204 SQL operations across 24 modules. Both QD and TL independently rejected it, citing the same critical risk: CableSnap's 666-line DB test suite mocks the entire SQLite layer, verifying SQL strings passed to `getAllAsync`/`getFirstAsync` — not actual query semantics or result correctness.
**Learning**: Tests that mock the database driver (intercepting SQL strings rather than executing them against a real or in-memory DB) provide zero regression safety for query refactoring. They verify "this code sends this SQL string" — which is exactly what changes during a migration. A query that produces identical results via different SQL will FAIL mock-based tests. A query that produces WRONG results via syntactically valid SQL will PASS them. Mock-based tests gate API contracts, not query correctness.
**Action**: Before attempting any large-scale query refactoring (ORM migration, query optimization, SQL dialect change), verify that the test suite executes queries against a real database — even an in-memory one. If tests mock the DB layer, build integration tests first. Do not rely on "all tests pass" as a quality gate when the tests cannot detect semantic query regressions.
**Tags**: testing, database, mocking, integration-tests, query-refactoring, orm, drizzle, regression-safety

### Consolidate Single-Assertion it() Blocks to Free Test Budget Slots
**Source**: BLD-402 — Personal Exercise Stats in Workout Detail Drawer
**Date**: 2026-04-19
**Context**: CableSnap has a hard 1800 test budget enforced by pre-push hook (scripts/audit-tests.sh). Budget was full at 1800/1800. Two new test files were needed for a new hook and component. The developer consolidated existing source-string tests across 3 files by merging single-assertion `it()` blocks that test the same logical concern into multi-assertion `it()` blocks.
...
### test.each Reduces Source-Level Budget Without Losing Runtime Coverage
**Source**: BLD-457 — Test Budget Reclamation & Consolidation Plan (Phase 75)
**Date**: 2026-04-20
**Context**: CableSnap's test budget is grep-based — `scripts/audit-tests.sh` counts source-level `it()` and `test()` declarations, not runtime test executions. With 1795/1800 budget consumed, the team needed to reclaim ~100 slots. 27 files using `fs.readFileSync` for structural tests were identified as high-value consolidation targets (245 tests → potentially ~10 `test.each` blocks).
**Learning**: `test.each(cases)("%s", ...)` counts as ONE source-level `test()` declaration regardless of how many cases it contains, but Jest still runs each case as a separate test at runtime. This means `test.each` reduces the grep-based budget count while preserving the same runtime test count and coverage. This is fundamentally different from merging `it()` blocks (which reduces both source-level count AND runtime count).
**Action**: When reclaiming test budget, prefer `test.each` over `it()` merging for tests that share the same assertion logic across multiple inputs (e.g., source-reading structural tests, parameterized validation). Use descriptive test names: `test.each(cases)("%s: %s", ...)` so CI output identifies which case failed. Reserve `it()` merging for tests that are genuinely testing the same logical concern.
**Tags**: jest, testing, test-budget, test-each, parameterized, source-level-count, budget-reclamation, grep-based
