...
describe("Strava Integration — DB Operations", () => {
  it("exports all strava DB functions and types from index", () => {
    for (const fn of ["getStravaConnection", "saveStravaConnection", "deleteStravaConnection",
      "createSyncLogEntry", "markSyncSuccess", "markSyncSkippedDuplicate", "markSyncFailed",
...
jest.mock("../../lib/db", () => ({
  ...
  markSyncSkippedDuplicate: jest.fn(),
  ...
  getEffectivePromoCaption: jest.fn(),
  getShareSettings: jest.fn(),
  getSyncLogForSession: jest.fn(),
}));
...
describe("Strava Integration — Behavioral", () => {
  ...
  it("(BLD-2995) upload returns 403 with non-JSON body → treated as permanent config error", async () => {
    ...
    expect(result.status).toBe("queued");
    expect((result as any).error.code).toBe("auth_expired");
    expect(db.markSyncPermanentlyFailed).not.toHaveBeenCalled();
    expect(db.deleteStravaConnection).not.toHaveBeenCalled();

    // BLD-3063: Sentry.captureException IS still called for actionable 403s
    expect(Sentry.captureException).toHaveBeenCalled();
    // BLD-3063: structured warn log IS emitted for transient failures
    expect(Sentry.logger.warn).toHaveBeenCalledWith(
      "strava upload failed post-connect",
      expect.objectContaining({
        source: "strava_activity_sync",
        phase: "post-connect",
        status: "queued",
  ...
  it("(BLD-2995) reconcileQueue: permanent app_inactive/config error short-circuits immediately", async () => {
    ...
    expect(db.markSyncFailed).toHaveBeenCalledWith("s-retry-inactive", expect.stringMatching(/inactive/i));
  ...
  it("(BLD-1240-b) 409 + lookup returns empty → status=skipped (unresolved duplicate, no toast)", async () => {
  ...
  it("(BLD-1240-c) 409 + lookup fails → status=skipped (unresolved duplicate, no toast, no Sentry error)", async () => {
  ...
  it("(BLD-1240-d) reconcileQueue: resolved 409 marks entry synced; unresolved 409 calls markSyncSkippedDuplicate", async () => {
    ...
    expect(db.markSyncSkippedDuplicate).not.toHaveBeenCalled();
    expect(db.markSyncFailed).not.toHaveBeenCalled();
    expect(db.markSyncPermanentlyFailed).not.toHaveBeenCalled();
  });

  it("(BLD-1240-d2) reconcileQueue: unresolved 409 (lookup empty) calls markSyncSkippedDuplicate, not markSyncFailed", async () => {
    const sessionId = "dup-queue-unresolved";
    db.getStravaConnection.mockResolvedValue({ athlete_id: 1 });
    db.getPendingOrFailedSyncs.mockResolvedValue([
    ...
    // POST /activities → 409
    mockFetch.mockResolvedValueOnce({ ok: false, status: 409, text: async () => '{"message":"Record Already Exists"}' });
    // GET /athlete/activities → empty (unresolved)
    mockFetch.mockResolvedValueOnce({ ok: true, json: async () => [] });

    await strava.reconcileStravaQueue();

    expect(db.markSyncSkippedDuplicate).toHaveBeenCalledWith(sessionId);
    expect(db.markSyncSuccess).not.toHaveBeenCalled();
    expect(db.markSyncFailed).not.toHaveBeenCalled();
    expect(db.markSyncPermanentlyFailed).not.toHaveBeenCalled();
  });

  it("(BLD-1240-d3) skipped-duplicate is in terminal state — reconcileQueue does not retry it", async () => {
    // After syncSessionToStrava returns { status: "skipped" }, the DB entry is terminal
    // (synced status). getPendingOrFailedSyncs filters to pending/failed only, so the
    // skipped entry must never appear in a subsequent reconcile run.
    // Simulate: the second call to reconcileQueue sees an empty pending list.
    db.getStravaConnection.mockResolvedValue({ athlete_id: 1 });
    // Empty pending list — terminal entries are excluded by getPendingOrFailedSyncs
    db.getPendingOrFailedSyncs.mockResolvedValueOnce([]);

    const uploadActivitySpy = mockFetch;
    const callsBefore = uploadActivitySpy.mock.calls.length;

    await strava.reconcileStravaQueue();

    // No additional fetch calls — uploadActivity was never called for the terminal entry
    expect(uploadActivitySpy.mock.calls.length).toBe(callsBefore);
  });

  it("(BLD-1240-e) 409 with unrecognized external_id: status=skipped, logs warning (not error), no captureException", async () => {
  ...
  describe("(BLD-3063) uploadActivity — 403 handling", () => {
    ...
    it("uploadActivity omits only promo caption when strava_description_enabled=false", async () => {
      const sessionId = "s-no-desc";
      db.getStravaConnection.mockResolvedValue({ athlete_id: 1 });
      db.getSessionSets.mockResolvedValue([
        { exercise_name: "Bench", weight: 80, reps: 8, completed: true, set_type: "working" },
      ]);
  ...
  describe("updateActivityDescription", () => {
    ...
    it("happy path — PUT succeeds", async () => {
      mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ id: 123 }) });

      await strava.updateActivityDescription("123", "New description");
    ...
    it("401 → disconnects and throws", async () => {
      mockFetch.mockResolvedValueOnce({ ok: false, status: 401, text: async () => "Unauthorized" });

      await expect(strava.updateActivityDescription("123", "Desc")).rejects.toThrow("revoked");
      expect(db.deleteStravaConnection).toHaveBeenCalled();
    });

    it("403 app_inactive → throws StravaError(app_inactive), no Sentry", async () => {
      const Sentry = require("@sentry/react-native");
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 403,
      ...
      await expect(strava.updateActivityDescription("123", "Desc")).rejects.toMatchObject({
        name: "StravaError",
        code: "app_inactive",
      });
      expect(Sentry.captureException).not.toHaveBeenCalled();
    ...
    it("403 actionable → Sentry captured, queued", async () => {
      const Sentry = require("@sentry/react-native");
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 403,
    ...
    db.getEffectivePromoCaption.mockResolvedValue("Tracked with CableSnap · https://github.com/alankyshum/cablesnap");
      ...
      it("PUT on post-sync caption edit", async () => {
        ...
        db.getSyncLogForSession.mockResolvedValue({
          ...
          synced_at: 1000, // synced_at is before shareSettings.updated_at (2000)
        });
        // Mock getActivity to return a different description
        mockFetch
          .mockResolvedValueOnce({
            ok: true,
            json: async () => ({ description: "Old description" }),
          })
          .mockResolvedValueOnce({
            ok: true,
            json: async () => ({ id: "12345" }),
        ...
        const result = await strava.syncSessionToStrava("s-phase4");
        expect(result.status).toBe("synced");
        // Verify GET and PUT are called
        expect(mockFetch).toHaveBeenCalledTimes(2);
        expect(mockFetch.mock.calls[0][0]).toContain("/activities/12345");
        ...
        // Verify analytics
        expect(Sentry.logger.info).toHaveBeenCalledWith(
          "strava_description_updated",
          expect.objectContaining({ sessionId: "s-phase4", activityId: "12345" })
        );
      });

      it("PUT skipped when unchanged", async () => {
        db.getSyncLogForSession.mockResolvedValue({
          status: "synced",
          strava_activity_id: "12345",
          synced_at: 1000,
        ...
        mockFetch.mockResolvedValueOnce({
          ok: true,
          json: async () => ({ description: expectedDesc }),
        });

        const result = await strava.syncSessionToStrava("s-phase4");
        expect(result.status).toBe("synced");
        // Verify GET is called but PUT is skipped
        expect(mockFetch).toHaveBeenCalledTimes(1);
        expect(mockFetch.mock.calls[0][0]).toContain("/activities/12345");
        expect(mockFetch.mock.calls[0][1].method).toBe("GET");
      });

      it("409 resolution path updates description if different", async () => {
        db.getSyncLogForSession.mockResolvedValue(null);
        mockFetch
          .mockResolvedValueOnce({
            ok: false,
            status: 409,
            text: async () => '{"message":"Record Already Exists"}',
          })
          .mockResolvedValueOnce({
            ok: true,
            json: async () => [{
          ...
          .mockResolvedValueOnce({
            ok: true,
            json: async () => ({ description: "Old description" }), // getActivity
          })
          .mockResolvedValueOnce({
            ok: true,
            json: async () => ({ id: "12345" }), // updateActivityDescription
          });

        const result = await strava.syncSessionToStrava("s-phase4");
        expect(result.status).toBe("synced");
        expect(result.activityId).toBe("12345");
        // Verify sequence of fetch calls
        expect(mockFetch.mock.calls[0][1].method).toBe("POST");
        expect(mockFetch.mock.calls[1][0]).toContain("/athlete/activities"); // lookup
        expect(mockFetch.mock.calls[2][0]).toContain("/activities/12345"); // GET
        expect(mockFetch.mock.calls[2][1].method).toBe("GET");
        expect(mockFetch.mock.calls[3][0]).toContain("/activities/12345"); // PUT
        expect(mockFetch.mock.calls[3][1].method).toBe("PUT");
      });

      it("failure in description update is logged and does not fail the sync", async () => {
        db.getSyncLogForSession.mockResolvedValue({
          status: "synced",
          strava_activity_id: "12345",
          synced_at: 1000,
        ...
        const result = await strava.syncSessionToStrava("s-phase4");
        expect(result.status).toBe("synced"); // Still returns synced!
      });
    });
  });
});
...
describe("Strava Integration — Sentry lifecycle logs (BLD-523)", () => {
  ...
  it("connectStrava emits lifecycle logs and never leaks tokens or auth codes", async () => {
    ...
    expect(Sentry.logger.info).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({ flow: "strava_connect", step: "success" }),
    );
...
describe("Strava Integration — Friendly Error Mapping (BLD-505)", () => {
  ...
  describe("getStravaUserMessage", () => {
    ...
    it("maps config and app_inactive errors to distinct support messages", () => {
