...
describe("workoutCSV — bodyweight_modifier_kg column (BLD-541)", () => {
  ...
  it("header includes bodyweight_modifier_kg as the last column", () => {
    const out = workoutCSV([]);
    expect(out.split("\n")[0]).toBe(
  ...
  it.each([
    { name: "positive modifier", modifier: 20, expected: "20" },
  ...
  ])("serializes $name as \"$expected\"", ({ modifier, expected }) => {
