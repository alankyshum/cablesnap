...
function makeSet(overrides: Partial<SetWithMeta> & { id: string }): SetWithMeta {
  return {
    ...
    set_type: (overrides.set_type ?? "normal") as SetType,
    rpe: overrides.rpe ?? null,
    notes: null,
    previous: null,
    is_pr: false,
    link_id: overrides.link_id ?? null,
    round: null,
    tempo: null,
...
function makeGroup(overrides: Partial<ExerciseGroup> & { exercise_id: string; sets: SetWithMeta[] }): ExerciseGroup {
  return {
    ...
    link_id: overrides.link_id ?? null,
    is_voltra: false,
    is_bodyweight: overrides.is_bodyweight ?? false,
    trackingMode: overrides.trackingMode ?? "reps",
