...
// -----------------------------------------------------------------------------
// patchFdroidLibrarySources — expo-application ApplicationModule.kt FOSS stub
// -----------------------------------------------------------------------------
//
// BLD-4392 regression harness. The previous over-greedy anchor
// `/\n\s*AsyncFunction\("getInstallReferrerAsync"\)[\s\S]*?\n\s*\}\n\s*\}\n\n\s*private val packageName/m`
// consumed the ModuleDefinition close brace AND the `private val packageName`
// declaration that followed it, silently emitting broken Kotlin. The DEX
// purity gate then caught the failure ~18 layers deep. These tests pin the
// verified-good behavior against the exact upstream 55.x source and require
// the patcher to fail LOUDLY when the anchor no longer matches — so a future
// expo-application bump surfaces the drift at prebuild instead of at
// scheduled release.

// Verbatim expo-application@55.0.14 ApplicationModule.kt. If you upgrade the
// pinned expo-application version and this fixture no longer represents
// upstream, refresh it AND verify patchFdroidLibrarySources still matches.
const EXPO_APPLICATION_55_APPLICATION_MODULE_KT = `package expo.modules.application
          ...
          when (responseCode) {
            InstallReferrerClient.InstallReferrerResponse.OK -> {
              try {
                ...
                promise.reject("ERR_APPLICATION_INSTALL_REFERRER_REMOTE_EXCEPTION", "RemoteException getting install referrer information.", e)
                return
              } finally {
                isSettled = true
              }
            ...
            InstallReferrerClient.InstallReferrerResponse.FEATURE_NOT_SUPPORTED -> {
              isSettled = true
              promise.reject("ERR_APPLICATION_INSTALL_REFERRER_UNAVAILABLE", "The current Play Store app doesn't provide the installation referrer API.", null)
            }
...
function writeAppModuleFixture(tmp, contents) {
  ...
  const file = path.join(appDir, "ApplicationModule.kt");
  fs.writeFileSync(file, contents, "utf8");
  return file;
}

describe("patchFdroidLibrarySources — expo-application FOSS stub", () => {
  const originalFdroid = process.env.CABLESNAP_FDROID;
  beforeEach(() => {
    process.env.CABLESNAP_FDROID = "1";
  });
  afterEach(() => {
    if (originalFdroid === undefined) delete process.env.CABLESNAP_FDROID;
    else process.env.CABLESNAP_FDROID = originalFdroid;
  });

  test("no-op when CABLESNAP_FDROID is not set (Play build path unaffected)", () => {
    delete process.env.CABLESNAP_FDROID;
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fdroid-app-noop-"));
    try {
      const file = writeAppModuleFixture(tmp, EXPO_APPLICATION_55_APPLICATION_MODULE_KT);
      expect(() => patchFdroidLibrarySources(tmp)).not.toThrow();
      expect(fs.readFileSync(file, "utf8")).toBe(EXPO_APPLICATION_55_APPLICATION_MODULE_KT);
    } finally {
      rmDirRecursive(tmp);
    }
  });

  test("rewrites the getInstallReferrerAsync block to a FOSS stub and strips imports", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fdroid-app-ok-"));
    try {
      const file = writeAppModuleFixture(tmp, EXPO_APPLICATION_55_APPLICATION_MODULE_KT);
      ...
      // Imports removed
      expect(patched).not.toMatch(/^import com\.android\.installreferrer/m);
      // Stub present with exact FOSS shape
      expect(patched).toContain(
        `AsyncFunction("getInstallReferrerAsync") { promise: Promise -> promise.resolve("") }`,
      );
      // Structural invariants that the old regex destroyed
      expect(patched).toMatch(/private val packageName\s*\n\s*get\(\) = context\.packageName/);
      expect(patched).toMatch(/private val packageManager\s*\n\s*get\(\) = context\.packageManager/);
      expect(patched).toMatch(/class ApplicationModule : Module\(\)/);
      // No lingering InstallReferrer* proprietary symbols
      expect(patched).not.toMatch(/InstallReferrer(Client|StateListener|Response)/);
      // Brace balance (a cheap-but-effective structural parse check)
  ...
  test("is idempotent — a second pass over already-patched source still fails loudly", () => {
    // After the first pass the getInstallReferrerAsync block is a single-line
    // stub and no InstallReferrerClient import remains. The imports anchor MUST
    // now miss (there is nothing to strip) — the patcher is required to fail
    // rather than silently no-op, because a silent no-op is exactly how the
    // original bug hid.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fdroid-app-idem-"));
    try {
      writeAppModuleFixture(tmp, EXPO_APPLICATION_55_APPLICATION_MODULE_KT);
      patchFdroidLibrarySources(tmp);
      expect(() => patchFdroidLibrarySources(tmp)).toThrow(
        /FOSS source patch anchor missed/,
      );
    } finally {
      rmDirRecursive(tmp);
    }
  ...
  test("throws with a clear message when the upstream ApplicationModule.kt shape drifts (missing block)", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fdroid-app-drift-"));
    try {
      // Simulate an expo-application upgrade that removed the InstallReferrer
      // AsyncFunction entirely. The patcher must not silently pass.
      const drifted = EXPO_APPLICATION_55_APPLICATION_MODULE_KT.replace(
        /^ {4}AsyncFunction\("getInstallReferrerAsync"\) \{[\s\S]*?^ {4}\}$/m,
        "",
      );
      // imports still present so anchor 1 still matches; anchor 2 must fail.
      writeAppModuleFixture(tmp, drifted);
      expect(() => patchFdroidLibrarySources(tmp)).toThrow(
        /FOSS source patch anchor missed for expo-application: AsyncFunction\("getInstallReferrerAsync"\) body/,
      );
    } finally {
      rmDirRecursive(tmp);
    }
  ...
  test("throws when the InstallReferrer imports anchor drifts (upstream renamed package)", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fdroid-app-drift2-"));
    try {
      const drifted = EXPO_APPLICATION_55_APPLICATION_MODULE_KT.replace(
      ...
      writeAppModuleFixture(tmp, drifted);
      expect(() => patchFdroidLibrarySources(tmp)).toThrow(
        /FOSS source patch anchor missed for expo-application: import com\.android\.installreferrer/,
      );
    } finally {
      rmDirRecursive(tmp);
    }
