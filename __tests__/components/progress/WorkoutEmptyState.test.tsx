...
describe('WorkoutEmptyState', () => {
  ...
  it('description text style uses onSurface-derived color, not onSurfaceVariant (BLD-3657)', () => {
    ...
    // Flatten the style array passed to the RNText node
    const flatStyles = Array.isArray(desc.props.style)
      ? desc.props.style.flat(Infinity)
      : [desc.props.style]
    ...
    // Must contain the onSurface base value (#000000) with an alpha suffix (CC = ~80%)
    expect(color).toBe('#000000CC')
    // Must NOT be the onSurfaceVariant color
