...
| Screenshot + optional VL description + tap bounds | `bash .claude/skills/cablesnap--android-emulator/scripts/look.sh <label> "<question>"` | Driving from a text-only shell, or to grab tap-bound coordinates for a visual element uiautomator does not expose. |
...
When driving from a text-only shell and you want a screen described in words, a local Ollama vision-language model can do it. Optional — if you can view images directly, just read the saved PNG.
