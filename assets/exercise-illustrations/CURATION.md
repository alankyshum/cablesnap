...
## Curation Protocol Supersession (BLD-989)

> **Note (2026-05-02):** The per-exercise two-gate sports-science model described above
> (visual plausibility + technique sign-off via `review--sports-science` per exercise)
> has been **superseded** for the 128-exercise Gemini set adopted in BLD-989.
>
> The replacement protocol (per QD verdict on BLD-989) is:
>
> 1. **Owner contact-sheet review** — the app owner reviews all 128 exercises using the
>    HTML contact-sheet tooling in `assets/exercise-illustrations/curation/`. The owner
>    posts a Paperclip comment on BLD-1005 with either `no red flags` or an explicit list
>    of flagged exercise IDs.
>
> 2. **Targeted technique sampling** — for high-risk movements (all `mw-cable-*` exercises,
>    and any `voltra-*` involving cable routing past the head/face/groin, including
>    `voltra-001` and `voltra-020`), the owner specifically inspects cable path, mount,
>    grip/attachment, and injury-risk pose.
>
> The two-gate process remains the documented standard for **future** generator runs
> producing new exercises beyond this 128-set. This note applies only to the batch
> landed via BLD-989 / BLD-1005.
>
> See: [BLD-1005](https://github.com/alankyshum/cablesnap/issues) (implementation issue)
