...
// removed from the canonical Exercise type. Use this alias at every call-site
// that touches mount_position so tsc doesn't reject it.
...
function buildAltTextEquipmentSummary(ex: SeedExercise): string {
  if (ex.equipment === "cable") {
...
function buildFrameCuesV2(ex: SeedExercise): FrameCues {
...
function buildCableVisibilityRulesV2(ex: SeedExercise): string[] {
  if (ex.equipment !== "cable") {
    return [];
  }
