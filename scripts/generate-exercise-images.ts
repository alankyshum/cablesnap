...
function preflightBinaries(): void {
  // Fail fast outside the per-exercise loop.
  try {
...
async function generateImagesFor(
  ex: ReturnType<typeof seedExercises>[number],
  exDir: string,
  apiKey: string,
  dryRun: boolean,
): Promise<void> {
...
async function processExercise(
  ex: ReturnType<typeof seedExercises>[number],
  cli: { forceRegen: boolean; dryRun: boolean; contactSheet: boolean },
  apiKey: string | undefined,
  manifestEntries: Map<string, { startAlt: string; endAlt: string }>,
): Promise<void> {
  const exDir = path.join(ASSET_DIR, ex.id);
  fs.mkdirSync(exDir, { recursive: true });
  const promptHash = sha256(fingerprintInput(ex));
  const existing = readFingerprint(exDir);
  const haveFiles =
    fs.existsSync(path.join(exDir, "start.webp")) &&
    fs.existsSync(path.join(exDir, "end.webp"));
...
async function main(): Promise<void> {
  ...
  if (!cli.dryRun) preflightBinaries();
