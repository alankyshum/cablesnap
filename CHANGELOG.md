...
- **F-Droid prebuild source-strip is now exhaustive (walks the tree)** instead of a hand-maintained file list, and R8 has explicit `-dontwarn` rules for proprietary classes on the F-Droid variant, resolving build failures on the `Wear OS UI tests` workflow. ([BLD-4377](/BLD/issues/BLD-4377))
