...
- [ ] Given I long-press the picker title and enter `20`, when I reopen the picker, then the grid shows 1–20 and `max_pulley_pins=20` persists for that exercise. [TODO-test: BLD-1123-followup]
...
- [ ] Given I save a set with `pulley_pin=6` and start a new session of the same exercise, when I view the next set row, then "Last session" shows `Pin 6` and tapping pre-fill sets `pulley_pin=6` on the new set. [TODO-test: BLD-1123-followup]
...
- [ ] Given I disable "Pulley pin tracking" in Settings, when I open any set row, then no pin chip renders. Existing `pulley_pin` data is preserved. [TODO-test: BLD-1123-followup]
...
- [ ] PR passes all tests with no regressions. [TODO-test: BLD-1123-followup]
- [ ] No new lint warnings. [TODO-test: BLD-1123-followup]
- [ ] **Android FullBackupContent lint passes** with both `<include>` siblings present in BOTH generated XML files (`data_extraction_rules.xml` AND `full_backup_content.xml`). `lintVitalRelease` CI gate runs on the PR. [TODO-test: BLD-1123-followup]
- [ ] CHANGELOG `## Unreleased` entry added. [TODO-test: BLD-1123-followup]
- [ ] Settings → Storage Usage surface shows "Setup photos: X MB" computed from on-disk size of `set-media/setup-*.jpg`. [TODO-test: BLD-1123-followup]
