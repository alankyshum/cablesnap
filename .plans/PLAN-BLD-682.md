...
- [ ] **AC1**: Given a workout template that was performed at least once before with set 1 = 100kg × 8, When the user starts a new session for that template and taps "+ Add Set" for the first set on that exercise, Then the WeightPicker shows `100` and the RepsPicker shows `8` without any user interaction. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC1: prefills weight + reps from previous workout when group has no in-session sets"`] [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"resolvePrefillCandidate > AC1: prev-workout fallback used when no in-session sets"`]
- [ ] **AC2**: Given the same conditions as AC1 and the user adds a second set, Then the second set inherits from the first in-session set (BLD-655 behaviour, unchanged). [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"resolvePrefillCandidate > AC1/AC2: in-session lastWorking takes priority over previous-workout fallback"`] [test: `__tests__/hooks/useSessionActions-add-set-prefill.test.ts::"useSessionActions — handleAddSet in-session prefill (BLD-655) > reps-tracking: copies weight + reps from last working set"`]
- [ ] **AC3**: Given an exercise has *no* prior session history, When the user taps "+ Add Set", Then the row appears with empty/zero pickers (silent no-op, no error log). [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC3: silent no-op when no in-session AND no prev-workout history"`] [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"resolvePrefillCandidate > AC3: silent no-op when no in-session AND no prev-workout source"`]
- [ ] **AC4**: Given a duration-tracked exercise (e.g. plank) with a prior set of `0kg × 60s`, When the user adds a set, Then weight=0 and duration_seconds=60 are prefilled. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC4: duration mode prefills weight + duration_seconds from prev workout"`] [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"resolvePrefillCandidate > AC4: duration mode shapes weight + duration_seconds (reps=null)"`]
- [ ] **AC5**: Prefilled values via the `handleAddSet` path are persisted via the existing `updateSet` write API. **Hydration prefill (display-only) does NOT write.** Verified by an `updateSet`-spy test: zero `updateSet` calls fire during session hydration of pristine rows; exactly one fires per `handleAddSet` invocation. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — no auto-prefill on session open (BLD-682 AC5) > AC5: zero updateSet calls fire on session-open hydration of pristine rows"`]
- [ ] **AC6**: If `updateSet` throws during `handleAddSet` prefill, the row insert still succeeds, no values are shown as "unsaved", and a single `console.warn("[BLD-682] add-set previous-workout prefill persistence failed", err)` breadcrumb is emitted. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC6: updateSet failure during prev-workout prefill → single console.warn, row remains visible"`]
- [ ] **AC7**: `colPrev` chip renders without text-overflow ellipsis on a 360dp viewport for **all** of the following inputs (Playwright web @ 360dp + Jest snapshot):
  ...
  Test asserts the rendered DOM does NOT contain a CSS-applied `text-overflow: ellipsis` truncation AND the literal substring (`1RM: 178` or `1RM: 1789`) is present. [TODO-test: BLD-1123-followup]
- [ ] **AC8**: All existing useSessionActions / SetRow / useSessionData tests pass; new unit tests cover AC1, AC3, AC4, AC6, AC7, AC11–AC15. [TODO-test: BLD-1123-followup]
- [ ] **AC9**: PR passes typecheck, lint, full test suite, and pre-push gates (LICENSE, illustration size, audit-tests, FTA <70 on changed files including `resolvePrefillCandidate` and `handleAddSet`). [TODO-test: BLD-1123-followup]
- [ ] **AC10**: GitHub #328 is updated when shipped, citing the version that contains the fix. [TODO-test: BLD-1123-followup]
- [ ] **AC11 — A11y label reads the *displayed* value (BLOCKER from QD rev-1 + rev-2)**. RNTL test mounts `SetRow` and asserts:
  ...
  - **Negative case (no data):** `set.weight=null, set.reps=null, prefillCandidate=null` → WeightPicker `Set 1 weight, 0 kilograms`; RepsPicker `Set 1 reps, 0`. No false-positive value. [test: `__tests__/components/session/SetRow-prefill-a11y.test.tsx::"BLD-682 AC11: a11y label reads displayed value (not nullable set.weight) > pristine row + prefillCandidate → weight label announces candidate value"`] [test: `__tests__/components/session/SetRow-prefill-a11y.test.tsx::"BLD-682 AC11: a11y label reads displayed value (not nullable set.weight) > persisted row → weight label announces set.weight, NOT prefillCandidate"`] [test: `__tests__/components/session/SetRow-prefill-a11y.test.tsx::"BLD-682 AC11: a11y label reads displayed value (not nullable set.weight) > no data anywhere → label reads 0 (no crash, no NaN, no 'null')"`]
- [ ] **AC12 — Partial prior set**. Given a previous session where `weight=100, reps=null, duration_seconds=null`, When the user adds a set, Then WeightPicker shows 100 and RepsPicker remains empty (NOT 0, NOT a default placeholder). Test asserts no `updateSet` call writes `reps=0`. [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"resolvePrefillCandidate > AC12: partial prior set leaves the missing field null (NOT 0)"`]
- [ ] **AC13 — Warmup-only history + lookup ordering**. Given the only logged sets in the prior session are warmups, When the user adds a working set, Then no prefill occurs (silent no-op) AND the warmup values are not written. Test pins the rule: lookup matches by `set_number` first, then filters out `set_type === 'warmup'`. Test asserts a `getPreviousSetsBatch` spy is called exactly once with the working set_number, and the warmup row is filtered in the helper, not in the SQL. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC13: warmup-only prev session → no prefill (warmup filtered by helper, not SQL)"`] [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"resolvePrefillCandidate > AC13: warmup prev-workout source returns null (no prefill)"`]
- [ ] **AC14 — Unit conversion at display**. Given prior set `weight=100` (canonical kg) and user's display unit = `lb` with WeightPicker step=5, When the user adds a set, Then the picker displays `220` (rounded to step), NOT `220.46` and NOT `100`. Test asserts the picker's `value` prop matches the rounded-to-step lb value. [test: `__tests__/components/session/SetRow-prefill-a11y.test.tsx::"AC14: SetRow is a pass-through display — \`unit\` prop swaps the WORD only > set.weight=45 + unit='lb' → label reads '45 pounds' (NOT silently converted)"`]
- [ ] **AC15 — Bodyweight modifier preserved**. Given a bodyweight exercise with prior set `bodyweight_modifier_kg=-20, reps=10`, When the user adds a new bodyweight set, Then `reps=10` is prefilled but `bodyweight_modifier_kg` stays at the BLD-541 default (or null) — NOT `-20`. Test asserts no `updateSetBodyweightModifier` call fires from the BLD-682 prefill path. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC15: bodyweight modifier prev-set is NOT carried over (BLD-541 default used)"`]
- [ ] **AC16 — Fallback ordering (techlead item 6)**. When in-session `lastWorking` exists, the previous-workout `getPreviousSetsBatch` MUST NOT be consulted. Test mounts a session with one in-session working set and asserts the `getPreviousSetsBatch` spy is NEVER called during a second add-set. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — handleAddSet prev-workout fallback (BLD-682) > AC16: in-session lastWorking present → getPreviousSetsBatch is NEVER called"`]
- [ ] **AC17 — Idempotence guard (techlead item 2)**. The hydration display-only path is keyed by `set.id` so re-renders do not redundantly recompute. Pristine guard checks `weight==null && reps==null && duration_seconds==null && completed==false && notes==null && bodyweight_modifier_kg==null`. Test double-mounts the hook with the same data and asserts `prefillCandidate` derivation is stable (no thrash). [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"derivePristinePrefillCandidate > AC17: pristine row + matching prev set → shaped candidate"`] [test: `__tests__/hooks/resolvePrefillCandidate.test.ts::"derivePristinePrefillCandidate > AC17 idempotence: same inputs produce equal outputs (referentially-pure)"`]
- [ ] **AC18 — Pristine-row completion persists candidate (techlead clarification + QD STRONG, rev-2)**. Given a pristine session row showing `prefillCandidate = { weight: 100, reps: 8 }`, when the user taps the set-number to mark complete WITHOUT touching the pickers, then `updateSet` is called exactly once with `{weight: 100, reps: 8}` BEFORE the completion write, and the persisted DB row reflects `weight=100, reps=8, completed=true`. Test asserts the order via mock-call timeline (`updateSet` call must precede the completion mutation). Without AC18, AC1's "values appear without any user interaction" promise silently breaks at the moment that matters most: the user thinks they logged 100×8; the DB stores nulls + completed=true. Same `single-write-path` (one `updateSet` call) and `write-on-intent` (set-completion is deliberate user action) — fully consistent with the option-B contract. [test: `__tests__/hooks/useSessionActions-add-set-prev-workout.test.ts::"useSessionActions — pristine-completion persistence (BLD-682 AC18) > AC18: completing a pristine row with prefillCandidate writes the candidate values BEFORE completion"`]
