...
- [ ] **AC6** Copy is verbatim what is in this plan §UX Design (PSY-1 Variant **B**: "You've logged RPE before. One tap after each set, and your rest adapts."). No loss-framing, FOMO, guilt, identity framing, streak language, or persuasive/manipulative imperatives. Reviewed by psychologist. [TODO-test: BLD-1123-followup]
- [ ] **AC7** Psychologist verdict APPROVED or APPROVED WITH CONDITIONS before any code merges. All conditions incorporated and re-confirmed on the final plan revision. [TODO-test: BLD-1123-followup]
...
- [ ] **AC10** A11y: Container uses `accessibilityRole="none"` (or omits role) — NOT `alert`. VoiceOver (iOS) AND TalkBack (Android) walk-through documented in PR description with screen recording. Container is reachable + labelled; both buttons reachable + labelled. [TODO-test: BLD-1123-followup]
...
- [ ] **AC12** PR passes all tests with no regressions. Typecheck clean. No new lint warnings. [TODO-test: BLD-1123-followup]
- [ ] **AC13** Build verified before `in_review` — `npm install`, typecheck, app boot. [TODO-test: BLD-1123-followup]
