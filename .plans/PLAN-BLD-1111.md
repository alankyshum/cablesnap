...
- [ ] **AC1** Given a device with `nudgeShown` unset AND `session.captureRpe ≠ "true"` AND the opened exercise has ≥ 1 `completed=1` `set_type != 'warmup'` workout set with non-null RPE, When the user opens the exercise via the Exercises tab (`ExerciseDetailPane`), Then the banner is visible above the muscle/illustration content. [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (a) renders banner when all 3 eligibility conditions hold"`] [test: `__tests__/components/exercises/ExerciseDetailPane-rpe-nudge.test.tsx::"ExerciseDetailPane RPE nudge mount > renders nudge when exercise has historical RPE and nudge not yet seen"`]
- [ ] **AC2** Given AC1 holds AND the user taps "Turn on", Then `session.captureRpe.nudgeShown="1"` is persisted FIRST, then `session.captureRpe="true"` is persisted, AND the banner unmounts, AND opening any exercise in the Exercises tab afterwards does NOT show the banner. [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (e) Turn on writes nudgeShown FIRST then captureRpe, calls onDismiss"`]
- [ ] **AC3** Given AC1 holds AND the user taps "Not now", Then `session.captureRpe.nudgeShown="1"` is persisted (and ONLY that), AND the banner unmounts, AND opening any exercise in the Exercises tab afterwards does NOT show the banner. [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (f) Not now writes ONLY nudgeShown, calls onDismiss"`]
- [ ] **AC4** Given `session.captureRpe="true"` (already opted in via Settings), When the user opens any exercise via the Exercises tab, Then the banner never renders. [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (c) does not render when captureRpe='true'"`]
- [ ] **AC5** Given the opened exercise has no `completed=1`, `set_type != 'warmup'` workout set with non-null RPE, When the pane opens, Then the banner never renders. Warmup-only RPE rows do NOT count. Incomplete-only RPE rows do NOT count. **`workout_sessions.kind='day_session'` (GTG) sets DO count** if they meet the predicate (intentional per TL-1; predicate test (f)). [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (d) does not render when no historical RPE"`] [test: `__tests__/lib/db/exercise-history-rpe-predicate.test.ts::"exerciseHasHistoricalRpe > (b) returns false when DB finds no completed set with rpe (warmup filter)"`] [test: `__tests__/lib/db/exercise-history-rpe-predicate.test.ts::"exerciseHasHistoricalRpe > (c) returns false when only incomplete rows have rpe (no matching row)"`] [test: `__tests__/lib/db/exercise-history-rpe-predicate.test.ts::"exerciseHasHistoricalRpe > (f) returns true for a day_session/GTG row with non-null rpe"`]
- [ ] **AC6** Copy is verbatim what is in this plan §UX Design (PSY-1 Variant **B**: "You've logged RPE before. One tap after each set, and your rest adapts."). No loss-framing, FOMO, guilt, identity framing, streak language, or persuasive/manipulative imperatives. Reviewed by psychologist. [gate: copy-review — psychologist-approved verbatim copy, no manipulative framing]
- [ ] **AC7** Psychologist verdict APPROVED or APPROVED WITH CONDITIONS before any code merges. All conditions incorporated and re-confirmed on the final plan revision. [gate: process — psychologist verdict APPROVED required pre-merge]
- [ ] **AC8** Given the user opens the pane (banner shows) but navigates away without tapping a button, When they re-open an eligible exercise, Then the banner shows again — closing without tapping does NOT burn the one-shot. (PSY affirmation: protects users not yet in Action stage.) [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (g) unmounts while predicate in-flight → no setState warning, no writes"`]
- [ ] **AC9** Given the user enables `session.captureRpe` via the Settings toggle (without ever seeing the banner), Then `session.captureRpe.nudgeShown="1"` is ALSO written by the Settings toggle handler. When they later toggle OFF in Settings AND open an eligible exercise, Then the banner does NOT render. (Hole closed by PreferencesCard one-line addition; covered by `PreferencesCard-rpe-nudge-suppression.test.tsx`.) [test: `__tests__/components/settings/PreferencesCard-rpe-nudge-suppression.test.tsx::"PreferencesCard RPE nudge suppression (AC9) > (a) toggling captureRpe ON calls markRpeCaptureNudgeSeen"`] [test: `__tests__/components/settings/PreferencesCard-rpe-nudge-suppression.test.tsx::"PreferencesCard RPE nudge suppression (AC9) > (b) toggling captureRpe OFF does NOT call markRpeCaptureNudgeSeen"`]
- [ ] **AC10** A11y: Container uses `accessibilityRole="none"` (or omits role) — NOT `alert`. VoiceOver (iOS) AND TalkBack (Android) walk-through documented in PR description with screen recording. Container is reachable + labelled; both buttons reachable + labelled. [gate: manual-a11y — VoiceOver+TalkBack walk-through with screen recording in PR]
- [ ] **AC11** Per-device, per-install one-shot. There is NO scheme for re-showing later. (PSY affirmation: any future re-prompt requires fresh psych review and a real fresh-start landmark.) [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (b) does not render when nudgeShown=true"`]
- [ ] **AC12** PR passes all tests with no regressions. Typecheck clean. No new lint warnings. [gate: ci — typecheck + test + lint pass required to merge]
- [ ] **AC13** Build verified before `in_review` — `npm install`, typecheck, app boot. [gate: ci — npm install + typecheck + app boot verified pre-review]
- [ ] **AC14** Tap idempotency: double-tap on either button results in exactly one set of writes. Buttons visibly disable while `writeInFlight=true`. Covered by RpeCaptureNudge test (i). [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (i) double-tap Turn on writes captureRpe exactly once"`]
- [ ] **AC15** Partial-failure UX: `nudgeShown` write fails → "Couldn't save — try again" toast + banner stays + no `captureRpe` write. `captureRpe` write fails after `nudgeShown` succeeded → "Saved your dismissal but couldn't enable capture — open Settings to retry" toast + banner unmounts. Covered by RpeCaptureNudge tests (j) and (k). [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (j) partial failure: nudgeShown throws → toast Couldn't save, banner stays"`] [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (k) partial failure: captureRpe throws after nudgeShown succeeded → banner unmounts"`]
- [ ] **AC16** Predicate failure observability: a DB error in `exerciseHasHistoricalRpe` writes a row to `error_log` AND the banner does not render AND the pane renders normally. Covered by RpeCaptureNudge test (h) and predicate test (g). [test: `__tests__/components/exercises/RpeCaptureNudge.test.tsx::"RpeCaptureNudge > (h) DB error in predicate → renders nothing"`] [test: `__tests__/lib/db/exercise-history-rpe-predicate.test.ts::"exerciseHasHistoricalRpe > (g) returns false and attempts error_log insert when DB throws"`]
