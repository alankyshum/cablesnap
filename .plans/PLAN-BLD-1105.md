...
- [ ] **AC9** All new components meet a11y: each interactive element has role + label + 44pt touch target; destructive actions include `accessibilityHint`; disabled Record CTA exposes `accessibilityState={{ disabled: true }}` and the disabled helper copy via `accessibilityLabel`. [test: `__tests__/acceptance/form-clips-record.test.tsx`::"AC2a — Record CTA disabled: no completed sets (BLD-1105) > shows 'Log a workout set first' copy and the button is disabled"]
