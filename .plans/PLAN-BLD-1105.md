...
## Acceptance Criteria
- [ ] **AC1** Given an exercise with at least one completed `kind='workout'` set that has no live clip, When the user opens Exercise Details → Form clips, Then a primary "Record clip" button is visible and enabled in the header; tapping it opens FormVideoSheet bound to that most-recent-without-clip set (resolved via `getMostRecentCompletedSetForExercise(id, { mustHaveNoClip: true })`, which returns `{ id: string, set_number: number, completed_at: number } | null`). [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC1 — Record CTA tap opens FormVideoSheet (BLD-1105) > opens FormVideoSheet in add mode with the free-set id when Record is tapped"`] [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC1 — Record CTA tap opens FormVideoSheet (BLD-1105) > also opens FormVideoSheet when the empty-state 'Record a clip' button is tapped"`]
- [ ] **AC2a** Given an exercise with zero completed `kind='workout'` sets, When the user opens the Form clips tab, Then the Record CTA is rendered in disabled state with helper copy "Log a workout set first to attach a form clip." [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC2a — Record CTA disabled: no completed sets (BLD-1105) > shows 'Log a workout set first' copy and the button is disabled"`] [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC2a — Record CTA disabled: no completed sets (BLD-1105) > does NOT open FormVideoSheet when a disabled Record CTA is pressed"`]
- [ ] **AC2b** Given an exercise where all completed sets already have clips, When the user opens the Form clips tab, Then the Record CTA is rendered in disabled state with helper copy "Replace or delete an existing clip below to record a new one." [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC2b — Record CTA disabled: all sets have clips (BLD-1105) > shows 'Replace or delete' copy and button is disabled"`]
- [ ] **AC3** Given a clip row in FormLibraryTab, When the user taps the row's overflow menu and selects Replace, Then FormVideoSheet opens in `mode='replace'` with `replaceTarget: { id: string, rel_path: string }` (where `id` is the `set_media.id` ULID); on successful Save the FormVideoSheet calls `saveReplacementClip({ oldId, oldRelPath, newClipArgs })` which (i) persists the new file via `persistRecordedClipFileOnly`, (ii) runs `withTransaction(async () => { await hardDeleteClip(oldId); newRow = await insertSetMedia(newMeta); })` as the atomic side-effect boundary — the inserted `SetMediaRow` is captured via an outer `let newRow: SetMediaRow | null` closed over by the callback (since `withTransaction` is `Promise<void>` per `lib/db/helpers.ts:187-195`) — and asserts `newRow !== null` after the tx resolves, (iii) best-effort `unlinkClipFiles(oldRelPath)` post-commit. FormVideoSheet then calls `onClipSaved(newRow.id)` (string) — the existing `onClipSaved: (clipId: string) => void` contract is preserved. No `SQLITE_CONSTRAINT_UNIQUE` error occurs. If the INSERT fails, the transaction rolls back (in-tx DELETE undone), the catch handler eagerly `unlinkClipFiles(newMeta.rel_path)` (best-effort) so the new file does not orphan, and the prior clip remains intact. [test: `__tests__/components/session/FormVideoSheet-replace.test.tsx`]
- [ ] **AC3b** Given a Replace flow where recording fails or is cancelled before Save, Then no DB mutation and no file unlink occurs; the prior clip is preserved verbatim. [test: `__tests__/lib/media/form-clips-replace-rollback.test.ts`]
- [ ] **AC4** Given a saved new or replaced clip, When recording completes, Then the FormLibraryTab grid refreshes (the new clip appears at top; replaced clips swap in place) and `onClipsChanged` fires. [test: `__tests__/components/session/FormLibraryTab-record.test.tsx`]
- [ ] **AC5** Given any number of clips, When the user opens Settings, Then the Form clips card is tappable, has a chevron, and tapping it opens FormClipsManageSheet. After the sheet dismisses, the card's stats refresh via `onClipsChanged` (count + bytes reflect any deletions). [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC5 — FormClipsStorageRow opens FormClipsManageSheet (BLD-1105) > tapping the row opens FormClipsManageSheet"`] [test: `__tests__/acceptance/form-clips-record.test.tsx::"AC5 — FormClipsStorageRow opens FormClipsManageSheet (BLD-1105) > closing the sheet refreshes stats and calls onClipsChanged"`]
- [ ] **AC6** Given the manage sheet open, When the user taps the trash icon on a row, Then an Alert confirms and on Confirm the clip is **soft-deleted** (preserves crash-recovery window) and removed from the list without dismissing the sheet; the sheet stat strip updates immediately. [test: `__tests__/components/settings/FormClipsManageSheet.test.tsx`]
- [ ] **AC7** Given multiple clips, When the user taps "Delete all clips" and confirms, Then every clip is **hard-deleted** (DB row removed AND file + thumbnail unlinked from `documentDirectory/form-clips/...`); `getStorageStats()` returns `{ count: 0, bytes: 0 }`; the sheet shows the empty state; the Settings card returns to "0 MB across 0 clips" via `onClipsChanged`. ENOENT during unlink does NOT abort the loop or leave the DB row behind. [test: `__tests__/lib/media/form-clips-bulk.test.ts`]
- [ ] **AC8** Given web platform, When Settings or FormLibraryTab render, Then no new affordances appear (existing `Platform.OS` guards extended). [gate: PR diff — no new platform-conditional branches; existing `Platform.OS !== 'ios'` guards in `FormLibraryTab.tsx` and `FormVideoSheet.tsx` cover this invariant]
- [ ] **AC9** All new components meet a11y: each interactive element has role + label + 44pt touch target; destructive actions include `accessibilityHint`; disabled Record CTA exposes `accessibilityState={{ disabled: true }}` and the disabled helper copy via `accessibilityLabel`. [test: `__tests__/acceptance/form-clips-record.test.tsx` — AC2a/AC2b tests assert `accessibilityState.disabled=true` and `accessibilityLabel` helper copy on disabled CTA; AC1 tests confirm enabled state; `__tests__/components/session/FormLibraryTab-record.test.tsx` covers enabled/disabled affordances]
- [ ] **AC10** PR passes `npm run typecheck`, `npm run lint`, `npm test`, and `npm run test:e2e` (existing form-clip e2e suite if any) with no regressions. [gate: CI — Typecheck, Jest, Vocab consistency audit, and Verify scenario hook CI checks all pass on PR #543]
- [ ] **AC11** No new lint warnings; no new Sentry breadcrumb categories beyond existing `form-clips`. [gate: CI lint pass on PR #543; no new Sentry categories introduced — confirmed by PR diff showing no new `Sentry.addBreadcrumb` calls]
- [ ] **AC12** No schema migrations introduced; `lib/db/schema.ts` byte-identical to main. [gate: PR diff — `lib/db/schema.ts` not touched in branch `bld-1123-record-btn-fix-acceptance-tests`; no new migration in `lib/db/migrations.ts`]
- [ ] **AC13** Manage sheet thumbnails reuse the existing FormLibraryTab placeholder/icon pipeline; no new thumbnail-generation code is added. [gate: PR diff — no new thumbnail code; `FormClipsManageSheet` reuses the same icon/placeholder components as `FormLibraryTab` grid rows; confirmed by `__tests__/components/settings/FormClipsManageSheet.test.tsx`]
