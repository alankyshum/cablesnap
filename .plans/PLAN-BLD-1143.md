...
## Acceptance Criteria
- [ ] Given a finished session with N completed sets, when the user opens the session summary, then a card titled **"Estimated pacing"** is rendered with **Working + Rest + Other** that sum to the session's gross duration (±1 s tolerance for rounding). [test: __tests__/lib/session-pacing.test.ts::"computePacing — pure logic (BLD-1144) > Other = gross - working - rest, clamped ≥ 0"] [test: e2e/scenarios/session-pacing.spec.ts::"@scenario session-pacing > PacingCard renders with literal title and segment labels (AC§134)"]
- [ ] The card title is exactly the literal string `"Estimated pacing"`. The three segment labels are exactly `"Working"`, `"Rest"`, `"Other"`. Locked via a new entry in `__tests__/source-contracts-batch.test.ts` (BLD-569 AC4 60-char gate respected). [test: __tests__/source-contracts-batch.test.ts::"PacingCard source contracts (BLD-1144) > title literal is exactly \"Estimated pacing\""] [test: __tests__/source-contracts-batch.test.ts::"PacingCard source contracts (BLD-1144) > segment labels are \"Working\", \"Rest\", \"Other\" (verbatim)"]
- [ ] The valenced words `"Idle"`, `"Wasted"`, `"Inactive"`, `"Off-task"`, `"Distraction"` MUST NOT appear in any **user-facing copy** for the feature. Asserted by a source-contracts test that scans only JSX text children and `accessibilityLabel`/`accessibilityHint` props in `components/session/summary/Pacing*.tsx` and the new pacing line in `components/history/DayDetailPanel.tsx` — NOT a whole-source substring grep (would false-positive on `countdown`, `dropdown`, etc., per techlead rev-2 note). The word "Down" is excluded from the list for the same reason; "Idle" and the other clearly valenced words remain forbidden. [test: __tests__/source-contracts-batch.test.ts::"PacingCard source contracts (BLD-1144) > forbidden word \"Idle\" absent from user-facing copy"] [test: __tests__/source-contracts-batch.test.ts::"PacingCard source contracts (BLD-1144) > forbidden word \"Wasted\" absent from user-facing copy"]
- [ ] Given the same session, when the user taps the PacingCard body, then a bottom sheet opens listing every exercise in the session with its **Working / Rest / Other** subtotals; tapping a column header sorts the list. [test: e2e/scenarios/session-pacing.spec.ts::"@scenario session-pacing > tapping PacingCard body opens breakdown sheet with per-exercise table (AC§137)"]
- [ ] Given the historical day-detail panel for a past session, when the panel renders, then a single one-line passive summary appears under the session title (`Working … · Rest … · Other …`); the line is **NOT a separate tap target** — it is plain text inside the existing row `Pressable`. [gate: manual-smoke — plain Text (no nested Pressable) verified by PR diff in components/history/DayDetailPanel.tsx; confirmed no separate Pressable wrapping the pacing line]
- [ ] Given a session with **zero** completed sets (e.g., started and abandoned), the PacingCard renders the literal message `"No completed sets to analyze"` and no chart — never crashes, never shows NaN, never blocks the rest of the summary screen. The empty-state copy must NOT include nudging language ("start logging", "try again", "get started", etc.) — locked via source-contracts. [test: __tests__/lib/session-pacing.test.ts::"computePacing — pure logic (BLD-1144) > empty session returns isEmpty=true with correct gross duration"] [test: __tests__/source-contracts-batch.test.ts::"PacingCard source contracts (BLD-1144) > empty-state copy is \"No completed sets to analyze\" — no nudging language"]
- [ ] Given any session, the Working number equals what `lib/rest-resolver.ts`'s `WORK_ESTIMATE_SECONDS_PER_REP × reps` (or `duration_seconds`) accounting would produce for the same set list — verified by a unit test that imports both modules and asserts equality on a fixture. [test: __tests__/lib/session-pacing.test.ts::"computePacing — pure logic (BLD-1144) > Working equals WORK_ESTIMATE_SECONDS_PER_REP × reps (single source of tru...)"]
- [ ] The TanStack Query cache key for pacing includes `session.edited_at` (with fallback to `completed_at` if null). Editing a completed session via the existing edit flow invalidates the pacing cache automatically — verified by a unit test. [test: __tests__/lib/session-pacing.test.ts::"useSessionPacing cache-key contract (BLD-1144) > cache key includes session.edited_at as editStamp (invalidates on edit)"]
- [ ] PR passes all tests with no regressions; net new test count documented in PR description and stays under the audit budget per `scripts/audit-tests.sh`. Current `it/test` count = **2890**, budget warn=2700, max=2900 — only **10 tests of headroom**. Pacing implementation will add **≤ 8 new tests** (5 pure-logic + 1 cache-key + 1 source-contract + 1 Playwright) and stay under 2900. If headroom is exceeded, bump max with a justification comment in `audit-tests.sh` per the stored convention; **bare `--no-verify` is forbidden**. [gate: ci — scripts/audit-tests.sh passes on merged PR; net new test count verified in PR description]
- [ ] No new lint warnings. [gate: ci — ESLint passes with zero new warnings on merged PR]
- [ ] Playwright scenario `e2e/scenarios/session-pacing.spec.ts` asserts the PacingCard is visible with the literal title `"Estimated pacing"` and the breakdown sheet opens on iPhone 14 (default `mobile` project). Per BLD-1124 convention (no per-scenario opt-in for extra viewports unless visual regression suspected), do not add Pixel/Fold steps in v1. [test: e2e/scenarios/session-pacing.spec.ts::"@scenario session-pacing > PacingCard renders with literal title and segment labels (AC§134)"] [test: e2e/scenarios/session-pacing.spec.ts::"@scenario session-pacing > tapping PacingCard body opens breakdown sheet with per-exercise table (AC§137)"]
- [ ] No notifications, no copy that could be classified as motivational, no streak / goal language anywhere in the feature. [gate: process — psychologist APPROVED WITH MODS (incorporated); no notification, streak, or goal code exists in feature scope per PR diff]
- [ ] All numeric labels formatted as `mm:ss` for sessions < 1 h, `h:mm:ss` for ≥ 1 h. [test: __tests__/lib/session-pacing.test.ts::"formatPacingTime (BLD-1144) > formats < 1h as mm:ss"] [test: __tests__/lib/session-pacing.test.ts::"formatPacingTime (BLD-1144) > formats ≥ 1h as h:mm:ss"]
- [ ] The ⓘ disclosure copy (verbatim): `"Working time is estimated as roughly 2 seconds per rep (or recorded duration for time-based sets). Rest is the remaining gap between consecutive sets."` Locked via source-contracts. [test: __tests__/source-contracts-batch.test.ts::"PacingCard source contracts (BLD-1144) > disclosure copy is verbatim AC§147"]
