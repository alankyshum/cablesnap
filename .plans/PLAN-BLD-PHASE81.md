...
- **Pain point observed:** Owner-reported session-screen reliability defects have accumulated as the dominant fix-cluster in the last shipping window — BLD-1257 (DB open failure mid-session), BLD-1235 (input persistence), BLD-1239 (finish-workout button), BLD-1260 (set handle contrast). All four touch the same flow: a user is in the middle of a workout, the session screen drops state, and the user loses entered data or has to redo the set.
- **Frequency:** Recurring theme over four consecutive bug fixes in the 0.26.x line. No single one is fatal, but together they signal a structural gap: **CableSnap currently has no mechanism to recover a workout-in-progress if the app crashes, is force-killed, or the device runs out of battery mid-session.**
- **Adjacent precedent:** PLAN-BLD-PHASE78 (Auto-Backup After Workout, BLD-466) added durability for *completed* workouts. This plan extends the same trust theme to *in-progress* workouts.
