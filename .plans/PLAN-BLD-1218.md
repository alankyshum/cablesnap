...
0. **Slice 0 — Throwaway feasibility spike (techlead).** Add `react-native-health-connect@3.5.0`, run `expo prebuild --platform android --clean`, run `./gradlew :app:assembleRelease` AND `./gradlew :app:assembleReleaseFdroid`. Grep both APKs: `androidx/health/connect/*` must be present in both, `com/google/android/gms/*` must be absent in `releaseFdroid`. Smoke-test HC permission flow on a real device (Z Fold6) or Android emulator with HC sideloaded. Discard the branch. **Output:** comment on this issue with grep counts + go/no-go.
   ...
   - **Executed split (2026-05-12, CEO routing decision on BLD-1219):** the Copilot CLI runtime has no Android toolchain, so Slice 0 is split across two issues — **BLD-1219** (techlead, CI-driven build + DEX-strings gate via `.github/workflows/spike-bld-1219.yml`, replicates `scheduled-release.yml:378-467` toolchain, uploads APK artifacts) and **BLD-1220** (CEO/owner, real-device `READ_WEIGHT` permission round-trip on Z Fold6 or Android 14+ emulator with HC sideloaded). Hard gate now reads: Slices 1–5 do NOT start until BOTH BLD-1219 AND BLD-1220 report green.
   - **BLD-1219 result (2026-05-12T22:39Z): GO** — run [25764368655](https://github.com/alankyshum/cablesnap/actions/runs/25764368655) green. DEX gate: `androidx/health/connect/*`=1001 in BOTH `release` and `releaseFdroid` flavors; `com/google/android/gms/*`=0 in `releaseFdroid` (hard-zero gate clean); release-flavor GMS count=452 unchanged from BASELINE (no regression). Plugin chain (`with-wearos-module` + `with-form-clips-backup` + Sentry) compatible with HC; HC SDK does NOT pull GMS transitively. CI half of Slice 0 unblocked. Awaiting BLD-1220 device smoke.
