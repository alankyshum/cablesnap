...
export type SetRowProps = {
  ...
  equipment: Equipment;
  onUpdate: (setId: string, field: "weight" | "reps" | "duration_seconds", val: string, side?: "left" | "right") => void;
...
export const SetRow = memo(function SetRow({
  ...
  const handleCopyLeftToRight = useCallback(() => {
    ...
    const performCopy = () => {
      if (leftWeight !== null) onUpdate(set.id, "weight", String(leftWeight), "right");
      if (leftReps !== null) onUpdate(set.id, "reps", String(leftReps), "right");
    };

    const hasRightValue = rightSet && ((rightSet.weight !== null && rightSet.weight > 0) || (rightSet.reps !== null && rightSet.reps > 0));
    if (hasRightValue) {
      Alert.alert(
  ...
  if (trackUnilateral && set.set_type === "normal") {
    const leftWeight = set.weight;
    const leftReps = set.reps;
    const rightWeight = rightSet ? rightSet.weight : null;
    const rightReps = rightSet ? rightSet.reps : null;
    ...
    const rightDisplayedWeight = rightSet ? (rightWeight ?? rightSet.prefillCandidate?.weight ?? null) : null;
    const rightDisplayedReps = rightSet ? (rightReps ?? rightSet.prefillCandidate?.reps ?? null) : null;
      ...
      <View testID={`set-${set.id}-row`} style={{ paddingVertical: 4 }}>
        <View
          style={[
            ...
            (set.completed || (rightSet && rightSet.completed)) && { borderColor: colors.primary },
          ]}
        >
          {/* SET Number */}
            ...
            <View style={{ flex: 1, marginHorizontal: 2 }}>
              {isBodyweight ? (
                <BodyweightModifierChip
                  ...
                  unit={unit}
                  onPress={() => rightSet && onOpenBodyweightModifier?.(rightSet.id)}
                  onLongPress={() => rightSet && onClearBodyweightModifier?.(rightSet.id)}
                  setNumber={set.set_number}
                />
              ) : (
                <SetWeightCell
                  setId={rightSet?.id ?? set.id}
                  setNumber={set.set_number}
                  ...
                  onWeightChange={(v) => {
                    if (rightSet) {
                      onUpdate(rightSet.id, "weight", String(v));
                    } else {
                      onUpdate(set.id, "weight", String(v), "right");
                    }
                  }}
                  onManualWeightSave={handleRightManualWeightSave}
                  onMarkerConfirm={(result) => onMarkerConfirm?.(rightSet?.id ?? set.id, result)}
                />
              )}
            </View>
            <View style={{ flex: 1, marginHorizontal: 2 }}>
              <WeightPicker
                ...
                onValueChange={(v) => {
                  if (rightSet) {
                    onUpdate(rightSet.id, "reps", String(v));
                  } else {
