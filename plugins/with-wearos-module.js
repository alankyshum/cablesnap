...
const RELEASE_FDROID_BUILD_TYPE = `
        ...
        releaseFdroid {
            ...
            // Variant-scoped R8 rules: silence "Missing classes detected"
            // errors for the proprietary Firebase / GMS / ML Kit / Install
            // Referrer packages that the F-Droid graph intentionally excludes
            // (see FDROID_EXCLUDES_BLOCK). Prebuilt Expo modules still carry
            // compile-time references to those types; without -dontwarn, R8
            // fails the releaseFdroid minify task. The rules file is written
            // by writeFdroidProguardRules() during the plugin's dangerousMod
            // step and lives at android/app/proguard-fdroid.pro. Using
            // proguardFile inside the releaseFdroid block keeps the rules
            // strictly variant-local — Play \`release\` shrinking is
            // untouched (AC10b: no proprietary classes end up in the dex;
            // -dontwarn only silences warnings, it does not keep code).
...
// R8 rules file emitted for the releaseFdroid variant. The file is referenced
// by `proguardFile "proguard-fdroid.pro"` inside RELEASE_FDROID_BUILD_TYPE so
// the rules only apply to that variant — the Play `release` shrinker is
// unchanged. `-dontwarn` tells R8 that missing references to the listed
// proprietary packages are intentional (they are excluded from the F-Droid
// classpath by FDROID_EXCLUDES_BLOCK / FDROID_APP_EXCLUDES_BLOCK), so R8's
// missing-class check stops failing the build. `-dontwarn` does NOT `-keep`
// anything, so AC10b (no proprietary classes in the final dex) is preserved.
const FDROID_PROGUARD_RULES = `# Generated by plugins/with-wearos-module.js during expo prebuild.
# DO NOT EDIT — regenerated every prebuild. See writeFdroidProguardRules().
#
# The F-Droid build (CABLESNAP_FDROID=1) removes proprietary GMS / Firebase /
# ML Kit / Install Referrer dependencies from the Gradle graph. Prebuilt Expo
# modules (expo-camera BarcodeAnalyzer, expo-notifications, expo-application)
# still contain compile-time references to those classes. R8 (enabled on the
# releaseFdroid build type via minifyEnabled true + shrinkResources true) then
# hard-errors with "Missing classes detected while running R8".
#
# -dontwarn silences those specific missing-class errors WITHOUT adding the
# classes back to the dex (no -keep). The shrinker still discards the now-
# unreachable Expo methods that referenced them, which is the whole reason
# minifyEnabled is on for this variant.
#
# Scope: variant-local (proguardFile inside releaseFdroid { ... }). The Play
# release shrinker never sees this file — verified in unit tests.
...
function writeFdroidProguardRules(platformRoot) {
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "proguard-fdroid.pro"), FDROID_PROGUARD_RULES, "utf8");
}
...
const withWearOsModule = (config) => {
  ...
  config = withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      // Write the variant-scoped R8 rules file referenced by
      // `proguardFile "proguard-fdroid.pro"` inside the releaseFdroid build
      // type. Same idempotency contract as the manifest overlay.
