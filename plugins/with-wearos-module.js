...
const FDROID_AUTOLINKING_EXCLUDES_MARKER = "// cablesnap:wearos:fdroid-autolinking-excludes";
...
const FDROID_AUTOLINKING_EXCLUDES_BLOCK = `
${FDROID_AUTOLINKING_EXCLUDES_MARKER}
if (System.getenv("CABLESNAP_FDROID") == "1") {
    expoAutolinking {
...
function patchSettingsGradle(contents) {
  if (
    ...
    (process.env.CABLESNAP_FDROID !== "1" || contents.includes(FDROID_AUTOLINKING_EXCLUDES_MARKER))
  ) {
    return contents;
  }
  ...
  if (
    process.env.CABLESNAP_FDROID === "1" &&
    !out.includes(FDROID_AUTOLINKING_EXCLUDES_MARKER)
  ) {
    if (out.includes("expoAutolinking.useExpoModules()")) {
      out = out.replace("expoAutolinking.useExpoModules()", `${FDROID_AUTOLINKING_EXCLUDES_BLOCK}\nexpoAutolinking.useExpoModules()`);
    } else if (out.includes("useExpoModules()")) {
      out = out.replace("useExpoModules()", `${FDROID_AUTOLINKING_EXCLUDES_BLOCK}\nuseExpoModules()`);
    } else {
      out += FDROID_AUTOLINKING_EXCLUDES_BLOCK;
    }
  }
  return out;
}
...
function findLocalMavenFiles(dir, files = []) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      findLocalMavenFiles(fullPath, files);
    } else if (entry.isFile()) {
      if (fullPath.includes("local-maven-repo") && (entry.name.endsWith(".pom") || entry.name.endsWith(".module"))) {
        files.push(fullPath);
      }
    }
  }
...
function scrubPomDependencies(contents) {
  const dependencyRegex = /^[ \t]*<dependency>([\s\S]*?)<\/dependency>\r?\n?/gm;
  return contents.replace(dependencyRegex, (match, body) => {
    const groupMatch = body.match(/<groupId>\s*([^<\s]+)\s*<\/groupId>/);
    const artifactMatch = body.match(/<artifactId>\s*([^<\s]+)\s*<\/artifactId>/);
...
function scrubModuleDependencies(contents) {
  try {
    const data = JSON.parse(contents);
    if (Array.isArray(data.variants)) {
      for (const variant of data.variants) {
        if (Array.isArray(variant.dependencies)) {
          variant.dependencies = variant.dependencies.filter((dep) => {
...
function patchFdroidExpoDependencies(projectRoot) {
  ...
  // Scrub F-Droid SUSS `<dependency>` blocks from POM and `.module` metadata under local-maven-repo
  const nodeModulesDir = path.join(projectRoot, "node_modules");
  if (fs.existsSync(nodeModulesDir)) {
    const mavenFiles = findLocalMavenFiles(nodeModulesDir);
    for (const file of mavenFiles) {
      const original = fs.readFileSync(file, "utf8");
      let patched = original;
      if (file.endsWith(".pom")) {
        patched = scrubPomDependencies(original);
      } else if (file.endsWith(".module")) {
        patched = scrubModuleDependencies(original);
      }
      if (patched !== original) {
...
// ---------------------------------------------------------------------------
// patchFdroidSourceFiles — SOURCE-LEVEL proprietary class stripping (BLD-4226)
// ---------------------------------------------------------------------------
//
...
//                        ExpoHandlingDelegate (patched not replaced)
//                        NotificationSerializer (patched not replaced)
//   expo-application   — ApplicationModule (patched not replaced)
//
...
//
function patchFdroidSourceFiles(projectRoot) {
  if (process.env.CABLESNAP_FDROID !== "1") return;

  // Mutate package.json to exclude expo-notifications and expo-application from autolinking under CABLESNAP_FDROID=1
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    if (!pkg.expo.autolinking.exclude.includes("expo-notifications")) {
      pkg.expo.autolinking.exclude.push("expo-notifications");
    }
    if (!pkg.expo.autolinking.exclude.includes("expo-application")) {
      pkg.expo.autolinking.exclude.push("expo-application");
    }
  ...
  writeFile(path.join(cameraAndroid, "analyzers", "BarcodeAnalyzer.kt"), `package expo.modules.camera.analyzers
...
// MLKit / GMS code paths removed to satisfy AC10b DEX-string gate (BLD-4226).
...
`);
  ...
  writeFile(path.join(cameraAndroid, "analyzers", "MLKitBarcodeAnalyzer.kt"), `package expo.modules.camera.analyzers

import android.graphics.Bitmap
import expo.modules.camera.utils.BarCodeScannerResult

// F-Droid stub: MLKit barcode scanning not available (BLD-4226).
...
`);
  ...
  writeFile(path.join(cameraAndroid, "analyzers", "BarcodeScannerResultSerializer.kt"), `package expo.modules.camera.analyzers

import android.os.Bundle
import android.util.Pair
import expo.modules.camera.utils.BarCodeScannerResult

// F-Droid stub: MLKit barcode result serializer removed (BLD-4226).
...
// manifest via tools:node="remove" in the F-Droid manifest overlay (BLD-4226).
...
// F-Droid stub: Firebase messaging delegate — all operations are no-ops (BLD-4226).
...
`,
  ...
  // imports RemoteMessage, rewrite to remove that import.
  patchFile(
    path.join(notifAndroid, "service", "interfaces", "FirebaseMessagingDelegate.kt"),
    (src) => src.replace(/^\s*import com\.google\.firebase\.[^\n]+\n/gm, ""),
  );
...
// F-Droid stub: Firebase push tokens not available (BLD-4226).
...
// F-Droid stub: Firebase topic subscriptions not available (BLD-4226).
...
// F-Droid stub: Firebase-specific logging removed (BLD-4226).
object DebugLogging {
  @JvmStatic
...
// to compile its \`content is RemoteNotificationContent\` check (BLD-4226).
...
// F-Droid stub: Firebase notification trigger (BLD-4226).
...
`,
  ...
  patchFile(
    ...
    (src) => {
      ...
      out = out.replace(
        ...
        "\n      // F-Droid: Firebase trigger path removed (BLD-4226)",
      );
      return out;
    },
  );
  ...
  // -----------------------------------------------------------------------
  // expo-application — stub out installreferrer usage in ApplicationModule.kt
  // -----------------------------------------------------------------------
  patchFile(
    path.join(appAndroid, "ApplicationModule.kt"),
    (src) => {
      let out = src;
      // Remove InstallReferrerClient imports
      out = out.replace(/^\s*import com\.android\.installreferrer\.api\.[^\n]+\n/gm, "");
      // Replace getInstallReferrerAsync body with a stub that resolves with "".
      // The function signature is: AsyncFunction("getInstallReferrerAsync") { promise: Promise ->
      // ... complex body using InstallReferrerClient ... }
      // Replace the whole async function body.
      out = out.replace(
        /AsyncFunction\("getInstallReferrerAsync"\)[\s\S]*?(?=^\s*(?:AsyncFunction|Function|Events|Constants|Name|}\s*$))/m,
        `AsyncFunction("getInstallReferrerAsync") { promise: Promise ->
      // F-Droid stub: install referrer not available without Google Play (BLD-4226).
      promise.resolve("")
    }\n\n    `,
      );
      return out;
  ...
  // -----------------------------------------------------------------------
  // expo-notifications expo-module.config.json — remove the Firebase-backed
  // modules from the autolinking list so they are not registered with Expo's
  // module system (they're already stubbed above, but the config list should
  // reflect the F-Droid feature set).
  // -----------------------------------------------------------------------
  const notifModuleConfig = path.join(
    projectRoot, "node_modules", "expo-notifications", "expo-module.config.json",
  );
  if (fs.existsSync(notifModuleConfig)) {
    try {
      const config = JSON.parse(fs.readFileSync(notifModuleConfig, "utf8"));
      ...
      if (config.android && Array.isArray(config.android.modules)) {
        config.android.modules = config.android.modules.filter(
          (m) => !removeModules.has(m),
        );
      }
      fs.writeFileSync(notifModuleConfig, JSON.stringify(config, null, 2) + "\n", "utf8");
    } catch (_) {
      // If the config is malformed, leave it untouched.
    }
  }
}
...
const withWearOsModule = (config) => {
  ...
  config = withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      patchFdroidSourceFiles(projectRoot);
...
module.exports.patchFdroidSourceFiles = patchFdroidSourceFiles;
module.exports.findLocalMavenFiles = findLocalMavenFiles;
module.exports.scrubPomDependencies = scrubPomDependencies;
module.exports.scrubModuleDependencies = scrubModuleDependencies;
