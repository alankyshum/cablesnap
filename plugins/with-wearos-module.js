...
const RELEASE_FDROID_BUILD_TYPE = `
        ...
        releaseFdroid {
            ...
            // R8 must run for the ProGuard strip rules below to do anything.
            // The Expo/RN template gates \`release\`'s minifyEnabled on
            // \`enableProguardInReleaseBuilds\` (default false), so
            // \`initWith release\` inherits \`minifyEnabled false\`. We force it
            // ON here for the F-Droid variant only — Play (\`release\`) is
            // untouched. \`shrinkResources\` requires \`minifyEnabled\`, but we
            // deliberately leave it off to avoid resource-processing surprises
            // in the reproducible F-Droid build; class stripping alone is
            // what AC10b measures.
            minifyEnabled true
            // R8 ProGuard stripping for F-Droid. \`configurations.all { exclude
            // group: ... }\` only removes Maven-coordinate artifacts — it
            // CANNOT strip classes that \`expo-camera\` and \`expo-notifications\`
            // shade/bundle into their own AARs, because those classes land in
            // the DEX under the including library's coordinate. R8 performs
            // dead-code elimination across ALL DEX classes (including shaded
            // ones), so \`-assumenosideeffects\` + \`-dontwarn\` in
            // \`proguard-fdroid-strip.pro\` lets R8 eliminate Firebase / MLKit /
            // GMS tasks / install-referrer references as unreachable. The
            // rules file is written by \`writeFdroidProguardRules(platformRoot)\`
            // only when \`CABLESNAP_FDROID=1\`. This buildType is only assembled
            // by F-Droid's build server, so referencing the file
            // unconditionally here is fine — the file always exists in an
            // F-Droid prebuild, and the \`release\` (Play) buildType above does
            // not reference it, so Play builds are unaffected.
...
// ---------------------------------------------------------------------------
// F-Droid ProGuard/R8 strip rules — dead-code-eliminate shaded proprietary
// classes that coordinate-based Gradle exclusions cannot reach.
// ---------------------------------------------------------------------------
//
// Why this exists (see CEO direction on BLD-4124):
//   `configurations.all { exclude group: "com.google.mlkit" }` removes Maven
//   artifacts by coordinate. It CANNOT strip classes that `expo-camera` and
//   `expo-notifications` shade/bundle into their own AARs — those classes
//   land in the DEX under the including library's coordinate, so the
//   coordinate-level exclusions are no-ops for them.
//
// R8 (AGP's default shrinker on release-type builds) performs whole-program
// dead-code elimination across ALL DEX classes, including shaded ones.
// `-assumenosideeffects class com.google.firebase.** { *; }` tells R8 that
// calls into these classes have no observable side effects, so any code path
// that only calls into them (and whose result is unused) can be pruned. The
// F-Droid build has no live Firebase / MLKit / GMS-tasks / install-referrer
// code paths (barcode scanning is disabled and no push tokens are
// registered), so R8 eliminates them as unreachable.
//
// `-dontwarn` suppresses R8's "referenced class not found" warnings that
// would otherwise fail the F-Droid build when R8 encounters signatures that
// still reference the excluded classes before elimination completes.
//
// The rules file is only written when `CABLESNAP_FDROID=1` and only
// referenced from the `releaseFdroid` build type, so Play (`release`) builds
// are structurally unaffected.
const FDROID_PROGUARD_RULES = `# Auto-generated by plugins/with-wearos-module.js for CABLESNAP_FDROID=1.
# Do not edit by hand — regenerated on every \`expo prebuild\` with
# CABLESNAP_FDROID=1. See BLD-4124 for the rationale.
...
const withWearOsModule = (config) => {
  ...
  config = withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      // Write the F-Droid R8 strip rules file when CABLESNAP_FDROID=1. The
      // `releaseFdroid` build type unconditionally references
      // `proguard-fdroid-strip.pro`, so the file must exist for any F-Droid
      // assemble task. See writeFdroidProguardRules() and BLD-4124.
